...
class Point(object):
    ...
    def move_to(self,x,y):
        self.traveled =self.traveled + ((self.x - x) ** 2 + (self.y - y) ** 2) ** 0.5
        self.x=x
        self.y=y
        self.run=self.run+1
    ...
    def move_by(self,dx,dy):
        ...
        self.y=self.y+dy
        self.run=self.run+1
        self.traveled=self.traveled+(dy**2+dx**2)**0.5
    ...
    def get_distance_from(self,pt):
        distance=((self.x-pt.x)**2+(self.y-pt.y)**2)**0.5
        return distance

    def get_distance_from_start(self):
        distance=((self.initialx-self.x)**2+(self.initialy-self.y)**2)**0.5
        return distance
    ...
    def closer_to(self,pt2,pt3):
        d2=self.get_distance_from(pt2)
        d3=self.get_distance_from(pt3)
        if d2<d3:
            return pt3
        else :
            return pt2
